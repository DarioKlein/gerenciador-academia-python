...
class MatriculaView(ctk.CTkFrame):
    ...
    def __montar_listagem(self) -> None:
        matriculas = self.__matricula_service.listar()

        if not matriculas:
            ctk.CTkLabel(
                self.__conteudo,
                text="Nenhuma matrícula cadastrada no momento.",
        ...
        for linha_idx, matricula in enumerate(matriculas, start=1):
            aluno = self.__matricula_service.buscar_aluno(matricula.cod_aluno)
            modalidade = self.__matricula_service.buscar_modalidade(
                matricula.cod_modalidade
            )
            valores = (
                str(matricula.codigo),
                str(matricula.cod_aluno),
                aluno.nome,
                str(matricula.cod_modalidade),
                modalidade.descricao,
                str(matricula.qtde_aulas),
            )
